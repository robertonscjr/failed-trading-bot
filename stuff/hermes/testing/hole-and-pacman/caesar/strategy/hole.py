...
class Hole(Thread):
    ...
    def purchase_action(self):
	    ...
	    timestamp_range = timestamp - (300 * 5)
            candle = self.client.returnChartData(self.pair, 300, timestamp_range, timestamp)
	    have_green = self.candle_green(candle)
            
	    if not self.have_hole: #or not have_green: #or close_less_than_open:#or not self.growth_indicator(2,5,900):
                self.buy_order = False
	...
	box_closed  = (in_sell_box < 0.55)
        master_stop = (self.check_price_to_sell() <= self.buy_price) 
	...
	self.sold = self.was_sold()
        ...
        if (master_stop and not self.exist_difference(5) ):
 	    if not self.correct_place(0):
            ...
            stop = True #(self.check_price_to_sell() < self.buy_price * 1.012)
	...
	sell_orders = self.order_book['asks']
        ...
        price_to_sell = value_orders[0] - 0.00000010
	if price_to_sell < self._highest_bid():
    ...
    def check_price_to_surf(self):
        ...
        price_to_surf = self.buy_price * 1.02
