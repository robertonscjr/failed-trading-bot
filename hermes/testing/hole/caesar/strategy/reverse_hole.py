...
class ReverseHole(Thread):
    """ Reverse Hole Strategy.
    ...
    def __init__(self, client, pair, balance_percentage, update):
    ...
    def run(self):
        ...
        self.have_reverse_hole = (self._highest_bid() > 
                                  self._lowest_ask()*1.0025)
        while self.have_reverse_hole:
            try:
                self.update()
                self.buy()
                ...
                print "(%s) - Sell order executed at %.8f : %s" % (self.pair, 
                                                             self.sell_price, 
                                                                self._time())
    ...
    def purchase_action(self):
        ...
        self.have_reverse_hole = self._highest_bid() > self._lowest_ask()

        if not self.buy_order:
            if self.have_reverse_hole:
                self.buy_order = True
                self.buy()
    ...
    def buy(self):
        self.buy_price = self._lowest_ask()
        amount = client.returnOrderBook()[self.pair]['bids'][0][1] 
