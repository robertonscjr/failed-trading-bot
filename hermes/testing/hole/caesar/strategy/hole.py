...
class Hole(Thread):
    ...
    def __init__(self, client, pair, desired_hole, balance_percentage, update):
    ...
    def purchase_action(self):
        ...
        first_candle = self.client.returnChartData(pair, 900)[0]
    ...
    def buy(self):
        self.buy_price = self._highest_bid() + 0.00000014
...
#       if self._highest_bid() >= hi_threshold:
            if self.sell_order:
            ...
            stop = (self.sell_price <= self.buy_price * 1.01) #NOT_STOP
            print "Stop: %s - %s" % (stop, self.pair)

            if stop:
                pass
            else:
                if self.exist_difference():
                    if self.sell_order:
                        self.client.cancelOrder(self.sell_id)
                    self.sell_price = self.check_price_to_sell()
	            self.sell(self.sell_price)
                    ...
                    if sel.sell_order:
                        sefl.client.cancelOrder(self.sell_if)
                    self.sell_price = self.check_price_to_sell()
                    self.sell(self.sell_price)
                    print "(%s) - Sell order placed at %.8f : %s" %  
                                                          (self.pair, 
         ...
         #  if (not self.exist_difference() and not stop) \
         #  or not self.first_exec:
         #      if self.sell_order:
         #          self.client.cancelOrder(self.sell_id)
         # 
         #      self.sell_price = self.check_price_to_sell()
	...
	if (price_to_sell == value_orders[0]):
        ...
        price_to_sell = value_orders[0] + 0.00000004

        for i in range(number_of_orders - 1):
            if value_orders[i + 1] - value_orders[i] > 0.00000100:
                price_to_sell = value_orders[i+1] - 0.00000001
           # if value_orders[i + 1] / value_orders[i] >= 0.03:
            #    price_to_sell = value_orders[i+1] - 0.00000001
        return price_to_sell
