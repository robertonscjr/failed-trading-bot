...
def wma(chart_data, period):
    average = 0.0
    div = 0.0
    for i in range(period):
        average = average + float(chart_data[len(chart_data)-1-i]['close']) * float(chart_data[len(chart_data)-1-i]['volume'])
        div = div + float(chart_data[len(chart_data)-1-i]['volume'])
...
def ema(data, period, position=None, previous_ema=None):
#   ema = float(chart_data[len(chart_data) - int(period) - 1]['close'])
#
#   k = 2.0/(float(period) + 1.0)
#   for i in range(1, period):
#       close = float(chart_data[len(chart_data) - int(period) - 1 + i]['close'])
#       ema = (close - ema) * k + ema
#
#   return ema
...
#   window = period
#   data = [float(data['close']) for data in chart_data]
#
#   if len(data) < 2 * window:
#       raise ValueError("data is too short")
#   c = 2.0 / (window + 1)
#   current_ema = sma(chart_data[-window*2:-window], window)
