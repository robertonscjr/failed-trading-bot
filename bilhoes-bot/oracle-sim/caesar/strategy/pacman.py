...
#           amount = float(order_amount)
    ...
    def sale_action(self):
       ...
       #hi_threshold = self.buy_price * (1 + 0.03)
        ...
        ma_1 = poloniex.ema(chart_data, 8 * 4)
        ma_2 = poloniex.sma(chart_data, 26 * 4)
        ma_3 = poloniex.ema(chart_data, 21 * 4)
        ma = (ma_1 < ma_2 and ma_1 < ma_3)

        top, mid, bot = poloniex.bollinger(chart_data, 26 * 4) 

        last_close = float(chart_data[len(chart_data)-2]['close'])
 	if (ma and last_close < mid):
            cross = ma
            bollinger = last_close < mid
            stop = False # highest_bid <= lo_threshold or highest_bid >= hi_threshold

            self.caesar_log.log("(%s) - Cross: %s / Bollinger: %s / Stop: %s : %s" % \
                                                               (self.pair,
                                                                cross,
                                                                bollinger,
                                                                stop,
            ...
            self.caesar_log.log("(%s) - EMA(8): %.8f / SMA(26): %.8f / EMA(21): %.8f : %s" % \
                                              (self.pair,
                                               ma_1, 
                                               ma_2, 
                                               ma_3,
            ...
            self.caesar_log.log("(%s) - TOP: %.8f / MID %.8f / BOT: %.8f: %s" % \
                                               (self.pair,
                                                top, 
                                                mid, 
                                                bot,
